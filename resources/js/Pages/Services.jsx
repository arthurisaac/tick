...
export default function Services({ auth, url }) {
      ...
      <Flowbite>
        ...
        <div className='p-5'>
            ...
            <div className="grid grid-cols-3 grid-rows-2 gap-3 mt-10">
                {
                     services.map((service, index) => <Link href={route('services.info')} data={{ service: service.id }} key={index}>
                        <div className="relative max-w-xl mx-auto mt-6 ml-3 mr-3">
                            <img className="h-64 w-full object-cover rounded-md" src={`${url}/storage/${service.photo}`} alt="Random image" />
